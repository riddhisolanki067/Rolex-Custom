...
class SalesOrderCutting(SellingController):
...
def make_delivery_note(source_name, target_doc=None, kwargs=None):
	...
	mapper = {
		"Sales Order Cutting": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
	...
	if not kwargs.skip_item_mapping:
		mapper["Sales Order Item"] = {
			...
			"field_map": {
				"rate": "rate",
				"name": "custom_against_so_cutting_details",
				"parent": "custom_against_so_cutting",
			},
			"condition": lambda d: condition(d) and select_item(d),
			"postprocess": update_item,
		}

	so = frappe.get_doc("Sales Order Cutting", source_name)
	target_doc = get_mapped_doc("Sales Order Cutting", so.name, mapper, target_doc)

	if not kwargs.skip_item_mapping and kwargs.for_reserved_stock:
		sre_list = get_sre_details_for_voucher("Sales Order Cutting", source_name)
